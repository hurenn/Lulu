...
public class Character_Base : MonoBehaviour {
    ...
    protected virtual void _UpdateMotor() {
        ...
        // ジャンプ
        if (input.abilityB.pressed && _isGrounded) {
        ...
        // ジャンプリリース
        if ((!input.abilityB.held && _isJumping) || _currentJumpTime <= 0) {
            _isJumping = false;
        }
        // 長押しジャンプ
        if (input.abilityB.held && _isJumping) {
            velocity.y = _jumpForce;
    ...
    /// <param name="is_trap_damage">トラップなど、氷の無敵を貫通させるダメージかどうか</param>
