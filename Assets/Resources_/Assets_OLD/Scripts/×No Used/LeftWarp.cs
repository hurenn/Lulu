...
public class LeftWarp : MonoBehaviour
{
    float limittime = 0;
    ...
    void FixedUpdate()
    {
        ...
        //Debug.Log("WarpCool = " + WarpControl.cool + " LeftCool = " + cool + " left = " + left + " flag = " + flag);
        if (left == true)
            Left();
        if (upleft == true)
            Upleft();
        ...
        if (OnGround.jump == false)
        {
            ...
            if (exitKey == true && cool == false)
                if ((Input.GetKeyDown(KeyCode.Z) && Input.GetKey(KeyCode.LeftArrow)) || (Input.GetKey(KeyCode.Z) && Input.GetKeyDown(KeyCode.LeftArrow)))
                {
                    ...
                    {
                        Goal = GameObject.Find("Warp Default UpLeft").transform.position;
                        upleft = true;
                    }
                    else if (Input.GetKey(KeyCode.DownArrow))
                    {
                        Goal = GameObject.Find("Warp Default DownLeft").transform.position;
                        downleft = true;
                    }
                    else
                    {
                        Goal = GameObject.Find("Warp Default Left").transform.position;
                        left = true;
                    }
                }
        }
        ...
        if (dashtimer < PlayerInput.dashtime)
        {
            ...
            if (dash >= 2 && Input.GetKeyDown(KeyCode.LeftArrow) && cool == false)
            {
                WarpStart();
                if (Input.GetKey(KeyCode.UpArrow))
                {
        ...
        if (Input.GetKeyDown(KeyCode.LeftArrow) && dash == 0)
        {
            dashtimer = 0;
            dash = 1;
        }
        if (dash >= 1 && Input.GetKeyUp(KeyCode.LeftArrow))
        {
            dash = 2;
            dashtimer = 0;
        }
    ...
    void Upleft()//右下・右上に容易に壁抜けできるバグ Stayにしたら直ったぜ！！
    {
