...
public class UpWarp : MonoBehaviour
{
    float limittime = 0;
    ...
    void FixedUpdate()
    {
        ...
        if (dashtimer < PlayerInput.dashtime)
        {
            ...
            if (dash >= 2 && Input.GetKeyDown(KeyCode.UpArrow) && cool == false)
            {
                WarpStart();
                if (!Input.GetKey(KeyCode.RightArrow) && !Input.GetKey(KeyCode.LeftArrow))
                {
        ...
        if (Input.GetKeyDown(KeyCode.UpArrow) && dash == 0)
        {
            dashtimer = 0;
            dash = 1;
        }
        if(dash >= 1 && Input.GetKeyUp(KeyCode.UpArrow)){
            dash = 2;
            dashtimer = 0;
        }
    ...
    void Up()
    {
        ...
        switch (step)
        {
            ...
            case 1:
                if (inGround == true)
                {
                    if (GameObject.Find("Ride Check Up").GetComponent<RideCheck>().inGround == false)
                    {
                        step = 3;
                    }
                    transform.position = new Vector2(transform.position.x, transform.position.y - 1f);
